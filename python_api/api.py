...
async def lifespan(app: FastAPI):
    ...
    if hasattr(app_state["api"], 'playwright') and app_state["api"].playwright:
        await app_state["api"].stop_playwright()
...
async def get_video_data(url: str):
    ...
    async with app_state["lock"]:
        try:
            ...
            # --- ИЗМЕНЕНИЕ: Получаем ширину и высоту для точного разрешения ---
            ...
            video_details = {
                "resolution": f"{width}x{height}", # <-- Форматируем как ШИРИНАxВЫСОТА
                "fps": round(fps),
